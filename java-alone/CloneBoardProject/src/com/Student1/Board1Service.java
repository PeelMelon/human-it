...
Scanner scanner = new Scanner(System.in); // 키보드입력 받는 객체
	...
	public void selectAllBoard() {
		// 게시판 목록조회 화면구성
		System.out.println("-------------------------------------------------------------------------------------------");
		System.out.println("번호         제목                                 작성된 날짜    수정된 날짜              조회수   ");
		System.out.println("-------------------------------------------------------------------------------------------");
		...
		try {
			...
			while(rs.next()) {
				...
				System.out.println(
						...
						String.format("%-15s", rs.getDate("update_date") 
	...
	public void selectOneBoard() {
		...
		//조회수**
		String updateReadCountSql = "UPDATE board SET read_count = NVL(read_count, 0)+ 1 WHERE seq = ?";
	...
	public void deleteBoard() {
		...
		try {
			...
			if (rs.next()) {
				...
				if (resultRow == 0) {
					System.out.println("수정 할 게시글 번호가 없습니다.");
					return;
				}
			}
	...
	public void updateOneBoard() {
		...
		// 2. 게시글 특정번호 수정 sql
		String updateOneSql = "UPDATE board SET title = ?, content = ?, "
				+ "update_date = sysdate WHERE seq = ?";
